...
# Ligne ~35-50 environ (méthode _apply_transformation_effects)
    ...
    def _apply_healing_effects(self, hero, ability, log: List[str]) -> bool:
        ...
        if not any(keyword in description for keyword in ['soigne', 'regagne', 'soin', 'guéri', 'blessures', 'purification', 'résurrection']):
            return False
        ...
        if heal_amount <= 0 and heal_amount != -1:
            return False

        # === GESTION VALEURS SPÉCIALES ===
        if heal_amount == -1:  # Totalité des blessures
            heal_amount = hero.get_total_health()  # PV max complet
    ...
    def _extract_heal_amount(self, description: str, ability_name: str) -> int:
        """
        MÉTHODE CORRIGÉE - Extraction valeur de soin depuis description
        
        Patterns validés depuis les descriptions réelles :
        - "Soigne 2 blessures" → 2 PV
        - "Soigne 3 blessures" → 3 PV  
        - "Soigne 4 blessures" → 4 PV
        - "Soigne 5 blessures" → 5 PV
        - "Soigne 6 blessures" → 6 PV
        - "totalité des blessures" → PV max complet
        - "regagne X blessures" → X PV
        """
        import re
        
        description = description.lower()
        
        # === PATTERNS SPÉCIFIQUES VALIDÉS ===
        
        # 1. Totalité des blessures = soin complet
        if "totalité des blessures" in description:
            return -1  # Signal pour PV_max complet
        
        # 2. Patterns numériques précis : "soigne X blessures" ou "regagne X blessures"
        heal_patterns = [
            r'soigne(?:\s+jusqu\'à)?\s+(\d+)\s+blessures?',
            r'regagne(?:\s+jusqu\'à)?\s+(\d+)\s+blessures?',
            r'(\d+)\s+blessures?'  # Pattern générique en dernier
        ]
        
        for pattern in heal_patterns:
            heal_match = re.search(pattern, description)
            if heal_match:
        ...
        # Noms spécifiques identifiés
        if 'mineur' in name_lower:
            return 2  # Valeur par défaut pour soins mineurs
        elif 'majeur' in name_lower:
            return 5  # Valeur par défaut pour soins majeurs  
        elif 'supérieur' in name_lower:
            return 6  # Valeur par défaut pour soins supérieurs
        elif 'multiple' in name_lower:
            return 4  # Soin multiple d'Elneha
        elif 'résurrection' in name_lower:
            return -1  # Soin complet
        elif 'purification' in name_lower:
            return 2  # Purification de Raishi
        elif 'méditation' in name_lower:
            return 3  # Méditation de Raishi
        elif 'imposition' in name_lower:
            return 3  # Imposition des mains d'Atucan
        
        # === PATTERNS DE DÉGÂTS + SOIN (Vol de vie) ===
        
        # "Inflige X dégâts et soigne autant de blessures"
        damage_heal_match = re.search(r'inflige\s+(\d+)\s+dégâts.*soigne\s+autant', description)
        if damage_heal_match:
            return int(damage_heal_match.group(1))  # Vol de vie : même valeur que dégâts
        
        # === VALEUR PAR DÉFAUT ===
        
        return 2  # Valeur par défaut sécurisée pour les soins non identifiés
